...
proc ::board::updateToolBar_ {{menu} {varname} {mb ""} } {
  ...
  while {$i >= 0} {
    ...
    if {[info exists "${varname}($idx)"] } {
      #ToDo: maybe a better solution for getting the theme foreground color for the menu
      $menu entryconfigure $i -foreground [ttk::style lookup . -foreground "" black] -command "eval \$::${varname}($idx)"
    } else {
...
proc ::board::newToolBar_ {{w} {varname}} {
  ...
  ttk_menu $w.menu_back -font font_Regular
  set m $w.menu_back
  ...
  ttk_menu $w.menu_forw -font font_Regular
  set m $w.menu_forw
  ...
  ttk_menu $w.menu -font font_Regular
  set m $w.menu
