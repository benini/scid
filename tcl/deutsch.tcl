...
translate D Months {Januar Februar März April Mai Juni
...
menuText D FinderFileClose "Datei-Finder schließen" 0
...
menuText D TmtFileClose "Turnier-Finder schließen" 0
...
translate D MatchForAtLeast {Zutreffendend für mindestens}
translate D HalfMoves {Halb-Züge}
...
Sie können diesen Zug ersetzen - unter Verlust aller nachfolgender Züge - oder
...
ECO-Klassifikations-Ergebnisse sind nur temporär.
...
translate D ImportHelp1 {Eingeben oder Einfügen einer Partie im PGN-Format in
den oberen Rahmen.}
...
translate D CopyConfirm {
 Möchten sie wirklich die [thousands $nGamesToCopy]
...
Wollen sie die Version der Datenbank in dem neuen Format öffnen?
}
...
Dieser Vorgang kann eine Zeitlang dauern, muss aber nur einmal durchgeführt
...
<p>
Für zusätzliche Information bitte die anderen
...
<h4>Wie kann ich den ECO-Eröffnungs-Schlüssel der aktuellen Position
ermitteln?</h4>
<p>
...
<h4>Wie ändere ich die Spaltenbreite im Partieliste-Fenster?</h4>
<p>
Klicken Sie die jeweilige Spalten-Überschrift mit der linken oder
rechten Maustaste an und wählen Sie <b>Konfigurieren</b>.
</p>

<h4>Wie kann ich das Zugbaum-Fenster statt für die ganze Datenbank nur
...
und eine große Datenbank mit Großmeister-Partien. Wie kann ich eine
meiner Partien mit denen in der großen Datenbank vergleichen?</h4>
<p>
Öffnen Sie das <a Tree>Zugbaum-Fenster</a> in der großen Datenbank und
drücken Sie auf den
...
<li><menu>Finder</menu>: Öffnet das <a Finder>Datei-Finder</a>-Fenster.</li>
...
<ul>
  <li><menu>Wartungs-Fenster</menu>: Öffnet/schließt das
...
<li><menu>Turnier-Finder</menu>: Öffnet/schließt den
...
<li><menu>Zugbaum</menu>: Öffnet/schließt das <a Tree>Zugbaum-Fenster</a>.</li>
...
<br>
<li><menu>Eine PGN-Partie importieren</menu>: Öffnet das <a Import>Import-Fenster</a>,
...
<a Tree>Zugbaum-Fenster</a> das Eröfnungsrepertoire des Gegners
ansehen.
</p>
<p>
...
Wenn der Autoplay-Modus aktiviert und das Analyse-Fenster geöffnet ist,
...
welches das <term>WinBoard/Xboard</term>-Protokoll unterstützt.
</p>
<p>
...
<h4>Menüs des Kreuztabellen-Fensters</h4>
<p>
...
Repertoire-Dateien ist es die Anzahl der (gewählten oder ausgeschlossenen)
Varianten.
</p>
<p>
...
verändern Sie einfach die Größe des Partieliste-Fensters und wählen
...
<p>
Scids Import-Fenster ermöglicht es Ihnen, auf einfache Weise eine
...
<p>
Das Import-Fenster bietet auch eine weitere, bequeme Möglichkeit für
Änderungen an der aktuellen Partie: Sie können die aktuelle Partie in
das Import-Fenster einfügen (mit dem Schalter <b>Aktuelle Partie
...
sind, führt ein linker Mauklick zu einer Aktion. Zum Beispiel können Sie
...
Berichtsfensters (nicht verfügbar bei Windows-Versionen) schreibt den
Bericht in eine temporäre LaTeX-Datei, ruft <b>latex</b> für diese Datei
auf und führt dann <b>xdvi</b> zur Darstellung des Resultats aus. Das
...
<p>
Sie können die ELO-Entwicklung des Spielers graphisch dargestellt
...
<p>
Der <term>Turnier-Finder</term> ermöglicht es Ihnen, Turniere in der
...
<p>
Die Züge im Zugbaum-Fenster können nach Zügen (alphabetisch), ECO-Code,
...
<p>
Das Zugbaum-Fenster besitzt ein Dateimenü-Kommando und einen Schalter,
...
<p>
Das Zugbaum-Fenster enthält einen mit <term>Graphik</term> bezeichneten
...
<p>
Für jede geöffnete <a EPD>EPD-Datei</a> enthält das Zugbaum-Fenster
...
Sie bemerken, daß das Zugbaum-Fenster praktisch sofort aktualisiert
wird, wenn die gesuchte Stellung im Cachespeicher ist.
</p>
<p>
Das Zugbaum-Fenster hat ein Dateimenü-Kommando <term>Cache-Datei
...
Zugbaum-Fensters füllt die Cache-Datei mit Daten für viele
Eröffnungspositionen. Es werden etwa 100 der häufigsten
Eröffnungsstellungen gesucht, dann wird die Cache-Datei geschrieben.
</p>
<p>
Beachten Sie, daß eine Zugbaum-Cachedatei (.stc) völlig redundant ist;
...
<b>Partie-Information</b> wählen. Die Option "Ergebnis und bester Zug"
...
<p>
Wenn die Korrektur-Datei erst einmal geladen ist, können Sie sie mit
...
Elo-Zahl haben, für die die Schreibkorrektur-Datei aber eine Elo-Zahl
...
auf die Graphik oder drücken Sie die linke Cursortaste (oder [Entf]
oder [Rück]).
</p>
<p>
...
befindlichen Seite -- beachten Sie, daß sich dies vom Analyse-Fenster
...
<p>
Viele Optionen und Präferenzen in Scid (z.B. die Brettgröße, Farben,
...
werden in einer Optionen-Datei gesichert, wenn Sie im Optionen-Menü
...
in demselben Verzeichnis wie Scids Programmdatei. Für Benutzer von
...
Ergebnis, Datum, Indizes für Spieler-/Turnier-/Orts-Namen (die
...
<p>
Eine Suchoptions-Datei (Dateiendung: ".sso") enthält Einstellungen für
...
set tips(D) {
  ...
  {
    Wenn Sie einige Datenbank häufig öffnen, fügen Sie zu jeder ein
  ...
  }
  {
    ...
    <b>Cache-Datei füllen</b> im Dateimenü des Zugbaum-Fensters zu
  ...
  }
  {
    ...
    Ihrer Datenbank durchzuführen, da dies Scid ermöglichen wird,
    mehr Dubletten zu finden und zum Löschen vorzumerken.
  }
  {
  ...
  }
  {
    ...
    Spieler/Turnier/Ort/Runden-Namen Groß- und Kleinschreibung.
    Sie können sie aber berücksichtigen und gleichzeitig Wildcards
    verwenden (mit "?" = irgendein einzelnes Zeichen und "*" = null oder
    mehr Zeichen), wenn Sie den Suchtext in Anführungszeichen ("...")
    eingeben. Beispielsweise geben Sie "*BEL" (mit den Anführungszeichen)
