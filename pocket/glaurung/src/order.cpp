...
void order_moves(const position_t *pos, search_stack_t *ss, 
  ...
  for(ms=ss[ply].ms_ptr; ms<ss[ply+1].ms_ptr; ms++) {
    ...
    if((ms->move&0x1FFFF) == (pvmove&0x1FFFF)) {
      if(TO(m) == TO(ss[ply-1].move) && see(pos, FROM(m), TO(m)) > 0)
        ss[ply].last_moved_piece_hanging = true;
      ms->score = 1000000000; continue;
    }
    if(ms->move == ss[ply].mate_killer) {
      ms->score = 999999999; continue;
    }
    cap = CAPTURE(m); pc = PIECE(m); from = FROM(m); to = TO(m);
    if(PROMOTION(m) == QUEEN && see(pos, from, to) >= 0) {
      ms->score = 100000000 + cap; continue;
    }
    if(cap) {
      int s = see(pos, from, to);
      if(ply > 0 && s > 0 && to == TO(ss[ply-1].move)) {
        ms->score = 100000000 + s; ss[ply].last_moved_piece_hanging = true;
      }
      else if(s >= 0) ms->score = 90000000 + s;
      else ms->score = s;
    }
    else if(ms->move==ss[ply].killer) ms->score = 90000000;
    else if(ms->move==ss[ply].killer2) ms->score = 89999999;
    else if(ply >= 2 && ms->move==ss[ply-2].killer) ms->score = 89999998;
    else if(ply >= 2 && ms->move==ss[ply-2].killer2) ms->score = 89999997;
