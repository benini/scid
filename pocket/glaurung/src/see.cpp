...
bool global_see(const position_t *pos, int side, int min_value) {
  ...
  for(from=PieceListStart(pos, piece); from<=H8; from=NextPiece(pos, from)) {
    if(PawnRank[side][from] == RANK_7) return true;
    for(ptr = Directions[piece]; *ptr; ptr++) {
      to = from + (*ptr);
      if(PieceValues[pos->board[to]] >= min_value && 
         ColourOfPiece(pos->board[to]) == xside) {
        if(PieceValues[pos->board[to]]-P_VALUE >= min_value) return true;
        else if(see(pos, from, to) >= min_value) return true;
      }
    }
  }
  ...
  for(from=PieceListStart(pos, piece); from<=H8; from=NextPiece(pos, from))
    for(ptr = Directions[piece]; *ptr; ptr++) {
      ...
      if(PieceValues[pos->board[to]] >= min_value && 
         ColourOfPiece(pos->board[to]) == xside) {
        if(PieceValues[pos->board[to]]-N_VALUE >= min_value) return true;
        else if(see(pos, from, to) >= min_value) return true;
      }
    }
  ...
  for(from=PieceListStart(pos, piece); from<=H8; from=NextPiece(pos, from))
    for(ptr = Directions[piece]; *ptr; ptr++) {
      for(to = from + (*ptr); pos->board[to]==EMPTY; to += (*ptr));
      if(PieceValues[pos->board[to]] >= min_value && 
         ColourOfPiece(pos->board[to]) == xside) {
        if(PieceValues[pos->board[to]]-B_VALUE >= min_value) return true;
        else if(see(pos, from, to) >= min_value) return true;
      }
    }
  ...
  for(from=PieceListStart(pos, piece); from<=H8; from=NextPiece(pos, from))
    for(ptr = Directions[piece]; *ptr; ptr++) {
      ...
      if(PieceValues[pos->board[to]] >= min_value && 
         ColourOfPiece(pos->board[to]) == xside) {
        if(PieceValues[pos->board[to]]-R_VALUE >= min_value) return true;
        else if(see(pos, from, to) >= min_value) return true;
      }
    }
  ...
  for(from=PieceListStart(pos, piece); from<=H8; from=NextPiece(pos, from))
    for(ptr = Directions[piece]; *ptr; ptr++) {
      ...
      if(PieceValues[pos->board[to]] >= min_value && 
         ColourOfPiece(pos->board[to]) == xside) 
        if(see(pos, from, to) >= min_value) return true;
    }

  // King
  piece++;
  from = KingSquare(pos, side);
  for(ptr=Directions[piece]; *ptr; ptr++) {
    to = from + (*ptr);
    if(PieceValues[pos->board[to]] >= min_value && 
       ColourOfPiece(pos->board[to]) == xside) 
      if(!is_attacked(pos, to, xside)) return true;
  }
