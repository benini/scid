...
// PPC : Hash is in kB, not MB
   { "Hash", true, "256", "spin", "min 64 max 2048", NULL },
   ...
   { "History Threshold",   true, "80",   "spin",  "min 0 max 100", NULL }, // was 70, tuned for blitz play (PPC)
   ...
   { "Toga Exchange Bonus",  false, "30",    "spin",  "min 0 max 100", NULL }, 
   { "Toga King Safety", true, "true", "check", "", NULL },
   { "Toga King Safety Margin",  false, "1700",    "spin",  "min 500 max 3000", NULL },
   { "Toga Extended History Pruning",true, "true", "check", "", NULL }, // was false, tuned for blitz play (PPC)
   ...
   for (opt = &Option[0]; opt->var != NULL; opt++) {
      if (opt->declare) {
         if (opt->extra != NULL && *opt->extra != '\0') {
            send_msg("option name %s type %s default %s %s",opt->var,opt->type,opt->val,opt->extra);
         } else {
            send_msg("option name %s type %s default %s",opt->var,opt->type,opt->val);
         }
      }
   }
}
