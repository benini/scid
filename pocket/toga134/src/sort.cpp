...
static const int HistorySize = 12*64*64;//9*9*12*64*64; (PPC)
...
static void note_quiet_moves  (list_t * list, const board_t * board, int height);
...
static int  quiet_move_value  (int move, const board_t * board, int height);
static int  move_value_simple (int move, const board_t * board);

static int  history_prob      (int move, const board_t * board, int height);

static bool capture_is_good   (int move, const board_t * board);

static int  mvv_lva           (int move, const board_t * board);

static uint32  history_index  (int move, const board_t * board, int height);
...
int sort_next(sort_t * sort, int height) {
   ...
   while (true) {
      ...
      while (sort->pos < LIST_SIZE(sort->list)) {
         ...
         if (false) {
            ...
            if (!pseudo_is_legal(move,sort->board)) continue;
				sort->value = history_prob(move,sort->board,height);
      ...
      if (false) {
         ...
         note_quiet_moves(sort->list,sort->board,height);
         list_sort(sort->list);
...
void good_move(int move, const board_t * board, int depth, int height) {
   ...
   if (Killer[height][0] != move && !move_is_tactical(move,board)) {
   ...
   index = history_index(move,board, height);
...
void history_good(int move, const board_t * board, int height) {
...
void history_bad(int move, const board_t * board, int height) {
...
static void note_quiet_moves(list_t * list, const board_t * board, int height) {
   ...
   if (size >= 2) {
      for (i = 0; i < size; i++) {
         move = LIST_MOVE(list,i);
         list->value[i] = quiet_move_value(move,board,height);
      }
   }
}
...
static int move_value(int move, const board_t * board, int height, int trans_killer) {
   ...
   if (false) {
      ...
      value = quiet_move_value(move,board,height);
   }
...
#warning code changed to fit into Pocket PC
