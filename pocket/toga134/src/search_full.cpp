...
static int  full_quiescence      (board_t * board, int alpha, int beta, int depth, int height, mv_t pv[], bool in_pv);
...
/*   if (SearchBest[SearchCurrent->multipv].value == 0){
 ...
 //  }
   ...
   if (search_type == SearchShort || SearchInput->multipv > 0){
	   a = -ValueInf;
      b = +ValueInf;
   }
...
static int full_root(list_t * list, board_t * board, int alpha, int beta, int depth, int height, int search_type) {
   ...
   SearchStack[height].reduced = false;
	SearchStack[height].extended = false;
	SearchStack[height].capture = false;
...
/*	if (depth == 1){
		SearchStack[height].eval_value = full_quiescence(board,alpha,beta,0,height,new_pv,node_type==NodePV);
	} */
   ...
   for (i = 0; i < LIST_SIZE(list); i++) {
      ...
      new_depth = full_new_depth(depth,move,board,board_is_check(board)&&LIST_SIZE(list)==1,false,true, height);
		if (new_depth == depth)
			SearchStack[height].extended = true;
		else
			SearchStack[height].extended = false;
      ...
      if (search_type == SearchShort || best_value[SearchCurrent->multipv] == ValueNone) { // first move
         ...
         if (value > alpha) { // && value < beta
            ...
            SearchRoot->flag = false;
            search_update_root();
            value = -full_search(board,-beta-50,-alpha,new_depth,height+1,new_pv,NodePV);
...
static int full_search(board_t * board, int alpha, int beta, int depth, int height, mv_t pv[], int node_type) {
   ...
   if (depth <= 0 || node_type == NodePV){
	   if (node_type == NodePV)
			CheckDepth = 1 - CheckNb - 1;
	   else
			CheckDepth = 1 - CheckNb;
		if (depth <= 0){
			SearchStack[height].eval_value = full_quiescence(board,alpha,beta,0,height,pv,node_type==NodePV);
			return SearchStack[height].eval_value; 
		}
/*		else{
			SearchStack[height].eval_value = full_quiescence(board,alpha,beta,0,height,new_pv, node_type==NodePV);
			if(height >= 2){
				if (SearchStack[height].eval_value > SearchStack[height-2].eval_value+25 && SearchStack[height-1].extended == false){
					depth++;
				}
			}
		} */
...
// 		probe_depth = (2 * SearchCurrent->max_extensions) / 3;
// 		if ((board->piece_nb <=4 || height <= probe_depth)
//            && (height >= probe_depth 
// 		         || board->cap_sq != MoveNone || PIECE_IS_PAWN(board->square[MOVE_TO(SearchStack[height-1].move)]))
// 			  ) {
// 		
...
// 	}
	...
	if (recog_draw(board)) return ValueDraw;
   ...
   if (height >= HeightMax-1) return eval(board, alpha, beta, node_type==NodePV);
   ...
   if (UseNull && depth >= NullDepth && (node_type != NodePV || height%2 != 0)) {

      if (!in_check
       && !value_is_mate(beta)
       && do_null(board)
       && (!UseNullEval || depth <= NullReduction+1 || eval(board,alpha, beta, node_type==NodePV) >= beta)) {
         ...
         // null-move search
	      ...
	      move_do_null(board,undo);
			...
			SearchStack[height].move = SearchStack[height-1].move;
...
/*					else{ 
						if (!SearchStack[height-1].extended && node_type == NodePV)
							depth++;
					} */
            }
         }
         ...
         if (value >= beta && node_type != NodePV) {
   ...
   // mate threat 
/*   if (value <= VALUE_MATE(HeightMax-1)){ 
			if (!SearchStack[height-1].extended && node_type == NodePV)
							depth++;
   } */
   ...
   while ((move=sort_next(sort,height)) != MoveNone) {
...
/*		if (value > ((90 * 16384 + 50) / 100) && value < 16384 && new_depth < depth && SearchStack[height-1].extended == false && node_type == NodePV){
			new_depth++;
			SearchStack[height].extended = true;
		}*/

/*		if (new_depth < depth && SearchStack[height-1].extended == false && node_type == NodePV && played_nb < HistoryMoveNb){
			new_depth++;
			SearchStack[height].extended = true;
		}*/

      if (UseHistoryPruning && !in_check /* && depth < HistoryDepth */ && depth <= SearchCurrent->max_extensions/2 /*depth <= 4*/ 
			&& (node_type != NodePV /*|| UseExtendedHistory*/) 
			&& played_nb >= HistoryMoveNb /* && height%2 == 0 */
			&& new_depth < depth && value < (depth <= 2 ? HistoryValue / 2 : HistoryValue / 3)
			&& !move_is_dangerous(move,board) && !move_is_tactical(move,board)){ 
				continue; 
		}
		
		// futility pruning

	  if (UseFutility && depth <= 3 && node_type != NodePV) {
         ...
         if (!in_check && new_depth < depth && !move_is_tactical(move,board) && !move_is_dangerous(move,board)) {
            ...
            if (opt_value == +ValueInf) {
					...
					opt_value = eval(board,alpha,beta,node_type==NodePV) + FutilityMargin;
      ...
      if (UseHistory && depth >= HistoryDepth) {
         if (!in_check && played_nb >= HistoryMoveNb && new_depth < depth) {
            ...
            value = sort->value; // history score
            if (value < HistoryValue && (node_type != NodePV || (UseExtendedHistory && value < HistoryValue / 2)) &&
					 !move_is_dangerous(move,board) /* && !move_is_tactical(move,board)*/) {
   ...
   if (best_move != MoveNone) {
      ...
      if (best_value >= beta && !move_is_tactical(best_move,board)) {
         ...
         for (i = 0; i < played_nb-1; i++) {
            ...
            history_bad(move,board,height);
         }

         history_good(best_move,board,height);
      }
   }
...
static int full_quiescence(board_t * board, int alpha, int beta, int depth, int height, mv_t pv[], bool in_pv) {
   ...
   bool in_check;
   int old_alpha, probe_score;
   int value, best_value;
   int best_move;
   int move;
   int opt_value, probe_depth;
   attack_t attack[1];
   sort_t sort[1];
   undo_t undo[1];
   mv_t new_pv[HeightMax];
...
// 	}
   ...
   if (height >= HeightMax-1) return eval(board, alpha, beta, in_pv);
   ...
   if (!in_check) {
      ...
      value = eval(board, alpha, beta, in_pv);
...
/*    cd = CheckDepth;
   ...
   while ((move=sort_next_qs(sort)) != MoveNone) {
      ...
      move_do(board,move,undo);
      value = -full_quiescence(board,-beta,-alpha,depth-1,height+1,new_pv,in_pv);
      move_undo(board,move,undo);
...
static int full_new_depth(int depth, int move, board_t * board, bool single_reply, bool mate_threat, bool in_pv, int height) {
   ...
   if ((single_reply && ExtendSingleReply)
			|| (/*in_pv &&*/ MOVE_TO(move) == board->cap_sq // recapture
				 && (see_move(move,board) > 0 /*|| VALUE_PIECE(board->square[MOVE_TO(move)])==VALUE_PIECE(board->square[MOVE_FROM(move)])*/)) 
		//	|| (in_pv && MOVE_TO(move) == board->cap_sq) // recapture
		...
		//	|| (in_pv &&  board->square[MOVE_TO(move)] != PieceNone256 && 
		//	    SearchCurrent->max_extensions-height >= 6) 
			//|| (in_pv &&  board->square[MOVE_TO(move)] != PieceNone256 && SearchCurrent->max_extensions) 
...
/*	if (in_pv && board->square[MOVE_TO(move)] != PieceNone256 && see_move(move,board) > 0){
			return new_depth+1;
	} */

//	if (in_pv &&  board->square[MOVE_TO(move)] != PieceNone256 && height > 0 && (SearchStack[height-1].capture == false /* || height%2 != 0 */)){
//		SearchStack[height].capture = true;
//		return new_depth+1;
//	} 

/*	if (in_pv && mate_threat){
		if (height == 0)
			return new_depth+1;
		else if (SearchStack[height-1].extended == false)
			return new_depth+1;
	} */
