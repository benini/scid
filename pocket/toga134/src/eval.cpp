...
static const int QueenMobOpening = 1; //was 1
...
static const bool UseStorm = true; // was true
...
static const int PawnBlockedBishop = 10; 
...
static const int ProtectedPassedEndgame = 10;
...
static const int SSpaceWeight[16] = {
   0, 0, 15, 30, 60, 90, 90, 90, 90, 90, 90, 90 ,90, 90, 90, 90,
};
...
void eval_init() {
   ...
   MobUnit[White][BP] = 1; //MobAttack;
   MobUnit[White][BN] = 1; //MobAttack;
   MobUnit[White][BB] = 1; //MobAttack;
   MobUnit[White][BR] = 1; //MobAttack;
   MobUnit[White][BQ] = 1; //MobAttack;
   MobUnit[White][BK] = 1; //MobAttack;
   ...
   MobUnit[Black][WP] = 1; //MobAttack;
   MobUnit[Black][WN] = 1; //MobAttack;
   MobUnit[Black][WB] = 1; //MobAttack;
   MobUnit[Black][WR] = 1; //MobAttack;
   MobUnit[Black][WQ] = 1; //MobAttack;
   MobUnit[Black][WK] = 1; //MobAttack;
...
int eval(board_t * board, int alpha, int beta, bool in_pv) {
...
/*	if (egbb_is_loaded && board->piece_nb <=5 && ABS(SearchBest[0].value) >= WIN_SCORE) {
   ...
   if (COLOUR_IS_WHITE(board->turn)){
	...
	/* lazy Cutoff Thomas */
   if (LazyEval && board->piece_size[White] > 3 && board->piece_size[Black] > 3 && !in_pv){
...
static void eval_piece(const board_t * board, const material_info_t * mat_info, const pawn_info_t * pawn_info, int * opening, int * endgame) {
   ...
   for (ptr = &board->piece[me][1]; (from=*ptr) != SquareNone; ptr++) { // HACK: no king
      ...
      case Bishop64:
         ...
         op[me] += mob * BishopMobOpening - penalty;
         eg[me] += mob * BishopMobEndgame - penalty;
      ...
      case Rook64:
         ...
         // 7th rank
			...
			//if (PAWN_RANK(from,me) >= Rank7) op[me] += Rook7thOpening; 
...
/*         mob = -QueenUnit;
         ...
         // 7th rank
			...
			//if (PAWN_RANK(from,me) >= Rank7) op[me] += Queen7thOpening; 
      ...
      for (colour = 0; colour < ColourNb; colour++) {
            ...
            for (ptr = &board->piece[opp][1]; (from=*ptr) != SquareNone; ptr++) { // HACK: no king
               ...
               if (piece_attack_king(board,piece,from,king)) {
				...
				if (colour == 0){
					if (board->square[king+16] == BP) {
						piece_nb++;
						attack_tot += KingAttackUnit[BP];
					}
					if (board->square[king+(16+1)] == BP) {
						piece_nb++;
						attack_tot += KingAttackUnit[BP];
					}
					if (board->square[king+(16-1)] == BP) {
					...
					if (board->square[king+(16*2)] == BP) {
						piece_nb++;
						attack_tot += KingAttackUnit[BP];
					}
					if (board->square[king+(16*2+1)] == BP) {
						piece_nb++;
						attack_tot += KingAttackUnit[BP];
					}
					if (board->square[king+(16*2-1)] == BP) {
					...
					if (board->square[king-16] == WP) {
						piece_nb++;
						attack_tot += KingAttackUnit[WP];
					}
					if (board->square[king-(16+1)] == WP) {
						piece_nb++;
						attack_tot += KingAttackUnit[WP];
					}
					if (board->square[king-(16-1)] == WP) {
					...
					if (board->square[king-(16*2)] == WP) {
						piece_nb++;
						attack_tot += KingAttackUnit[WP];
					}
					if (board->square[king-(16*2+1)] == WP) {
						piece_nb++;
						attack_tot += KingAttackUnit[WP];
					}
					if (board->square[king-(16*2-1)] == WP) {
   ...
   for (colour = 0; colour < ColourNb; colour++) {
      ...
      for (bits = pawn_info->passed_bits[att]; bits != 0; bits &= bits-1) {
         ...
         ASSERT(COLOUR_IS(board->square[sq],att));
			...
			// Thomas protected passed pawn bonus

		   if (att == White){
	   		 if (board->square[sq-17] == WP || board->square[sq-15] == WP || board->square[sq-1] == WP || board->square[sq+1] == WP){
	   			op[att] += ProtectedPassedOpening; 
	   			eg[att] += ProtectedPassedEndgame;
				 }
			}
   		else{
	   		 if (board->square[sq+17] == BP || board->square[sq+15] == BP || board->square[sq-1] == BP || board->square[sq+1] == BP){
	   			op[att] += ProtectedPassedOpening; 
	   			eg[att] += ProtectedPassedEndgame;
				 }
			} 
