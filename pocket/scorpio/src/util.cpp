...
void mov_str(MOVE& move,char* s) {
	if(PIECE(m_piece(move)) != pawn) *s++ = piece_name[PIECE(m_piece(move))];
	*s++ = file_name[file(m_from(move))];
	*s++ = rank_name[rank(m_from(move))];
	if(m_capture(move)) *s++='x';
// else *s++ = '-'; PG: does not comply to xboard protocol
...
// same as mov_str but without the leading piece and x for capture and = for promotion
void mov_str_xboard(MOVE& move,char* s) {
//   if(PIECE(m_piece(move)) != pawn) *s++ = piece_name[PIECE(m_piece(move))];
  *s++ = file_name[file(m_from(move))];
  *s++ = rank_name[rank(m_from(move))];
//   if(m_capture(move)) *s++='x';
...
void print_pc(const int& pc) {
	send_msg("%c",piece_name[pc]);
}
void print_move(const MOVE& move) {
	char f[6],t[6];
	sq_str(m_from(move),f);
	sq_str(m_to(move),t);
	send_msg("%s %s %c %c %c ep = %d cst=%d",f,t,piece_name[m_piece(move)],
...
void SEARCHER::print_board() {
	...
	for(i = 7; i >= 0; i--) {
        send_msg("\t");
		for(j = 0;j < 8; j++) {
			send_msg("%c",piece_name[board[SQ(i,j)]]);
		}
		send_msg("\n");
	}
	...
	send_msg("play = %d opp = %d ep = %d cas = %d fif = %d hkey 0x"FMT64"\n\n",player,
...
void SEARCHER::print_history() {
	for(int i = 0;i < hply;i++) {
		...
		send_msg(" cst=%d ",hstack[i].castle);
		print_sq(hstack[i].epsquare);
		send_msg(" fif=%d chk=%d",hstack[i].fifty,hstack[i].checks);
        send_msg(" hkey 0x%I64x",hstack[i].hash_key);
		send_msg("\n");
	}
}
...
void SEARCHER::print_pv(int score) {
	...
	send_msg("%d %d %d %d ",search_depth,score,(get_time() - start_time)/10,nodes);
	/*print what we have*/
...
int SEARCHER::draw() {
	...
	if(fifty >= 101) {//avoid being mated on the 50th move
		return true;
	}
...
int SEARCHER::print_result() {
	...
	if(!legal_move) {
		...
		if(repetition >= 2) {
			...
			send_msg("1/2-1/2 {Draw by fifty move rule}\n");
			return R_DRAW;
        } else if(pawn_c[white] + pawn_c[black] == 0
			&& piece_c[white] + piece_c[black] <= 3) {
			send_msg("1/2-1/2 {Draw by insufficent material}\n");
			return R_DRAW;
        }    
	}
    return R_UNKNOWN;
...
void SEARCHER::init_data() {
		...
		if(!(sq & 0x88)) { 
			...
			if(pic != empty) {
				...
				if(PIECE(pic) == pawn) {
					...
					piece_c[COLOR(pic)]+=piece_cv[pic];
				}
				pcsq_score[COLOR(pic)] += pcsq[pic][sq];
				man_c[pic]++;
				all_man_c++;
			}
		}
	...
	root_score = (piece_c[white] + pawn_c[white] - piece_c[black] - pawn_c[black]) * 100;
...
void SEARCHER::set_board(const char* fen_str) {
	...
	for(r = RANK8;r >= RANK1; r--) {
        for(f = FILEA;f <= FILEH;f++) {
			sq = SQ(r,f);
			if(pfen = strchr(piece_name,*p)) {
				board[sq] = int(strchr(piece_name,*pfen) - piece_name);
			} else if(pfen = strchr(rank_name,*p)) {
...
void SEARCHER::COPY(SEARCHER* srcSearcher) {
	...
	for(i = 0;i < (ply + 1);i++) {
		...
		dstack->use_lazy = sstack->use_lazy;
...
sort move list
*/
void STACK::sort(const int start,const int end) {
	MOVE tempm;
	int temps,bi = start,bs = score_st[start],sc;
...
void CHESS_CLOCK::set_stime(int hply) {
	...
	if(max_sd != MAX_PLY) {
		search_time = MAX_NUMB;
		maximum_time = MAX_NUMB;
		return;
	}
	...
	if(!mps) {
		if(inc) {
			search_time = p_time / 35 + inc;
			maximum_time = p_time / 2;
		} else {
			search_time = p_time / 40;
			maximum_time = p_time / 4;
		}
	} else {
		int moves_left = (mps - ((hply/2) % mps));
	...
	p_time += 1000; 
	/*print time*/
	send_msg("[st = %.2fs, mt = %.2fs , hply = %d]\n",
		search_time / 1000.0f,maximum_time / 1000.0f,hply);
}
...
static char shared_buffer[MAX_STR];
...
// 		if(!(log_file = fopen(log_name ,"r"))) break;
// 		fclose(log_file);
// 	}
...
// 			int ch = _getche();
// 			if(ch == '\r') ch = '\n';
// 			shared_buffer[shared_index++] = (char)ch;
// 			if(ch == '\n') {
// 				shared_buffer[shared_index++] = 0;
// 				got_input = true;
// 				return 1;
// 			}
// 		}
...
// 	if(got_input) {
// 		memcpy(buffer,shared_buffer,MAX_STR);
// 		shared_index = 0;
// 		got_input = false;
// 		print_log(">");
// 		print_log(buffer);
// 		return true;
// 	}   
// 	if(fgets(buffer,MAX_STR,stdin)) {
// 		print_log(">");
// 		print_log(buffer);
// 		return true;
// 	}	
// 	return false;
  get_msg(buffer,MAX_STR);
  bool ret = ( buffer[0] != '\0' );
  return ret;
...
void SEARCHER::check_quit() {
	register int time_used = get_time() - start_time,delta;
	/*
	quit search if the input is different from "."
	*/
  get_msg( shared_buffer, MAX_STR );
//  if(bios_key()) {
  if (shared_buffer[0] != '\0') {
//    if(read_line(shared_buffer)) {

			if(shared_buffer[0] == '.') {
				char b[10];
				mov_str(stack[0].current_move,b);
				send_msg("stat01: %d %d %d %d %d %s\n",time_used / 10,nodes,
					search_depth,stack[0].count - stack[0].current_index - 1,stack[0].count,b);
				return;
			}
			got_input = true;
// 		}
	...
	delta = pr_score - r_score;
	
	if(delta <= -50 && abs(r_score) <= 700 ){
		factor = 2.0f;
	} else if(r_score <= 700) {
		if(delta < 25);
		else if(delta < 50)
	...
	if(time_used >= int(factor * chess_clock.search_time)) {
		abort_search = 1;
		return;
	}
}
...
*/
static UBMP64 read_bytes(int count) {
    UBMP64 x = 0;
	UBMP8* c = (UBMP8*) &x;
	for(int i = 0; i < count; i++) {
		c[i] = ((UBMP8) fgetc(book_file));
	}
	return x;
}

static void read_entry(PBOOK_E pbook_e,int middle) {

	fseek(book_file,8 + middle * 16,SEEK_SET);

	pbook_e->hash_key = (UBMP64) read_bytes(8);
	pbook_e->wins = (UBMP32) read_bytes(2);
	pbook_e->losses = (UBMP32) read_bytes(2);
	pbook_e->draws = (UBMP32) read_bytes(2);
	pbook_e->learn = (UBMP32) read_bytes(2);
}
...
void load_book() {
	...
	w_positions = (int) read_bytes(4);
	b_positions = (int) read_bytes(4);
	book_loaded = true;
}
...
MOVE SEARCHER::get_book_move() {
	...
	if(total_played)
		send_msg("  Move   Played     +        -        =     Learn  Sortv\n");
	...
	for(i = 0;i < pstack->count; i++) {
		...
		if(pbook_e->hash_key) {
			...
			score = (score * played * pbook_e->learn) / (total_played * 100);
			if(score) {
			...
			send_msg("%6s %8d %7.2f%% %7.2f%% %7.2f%% %6d %6d\n",mv_str,played,
