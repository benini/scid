...
// functions
// ####################################################
//char sockbufferw[512];
//char sockbufferr[256];
int sockfd = -1;
/*
#define SOCK_WRITE(a, b) sprintf(sockbufferw, a , b) ; \
	if (write(sockfd, sockbufferw,strlen(sockbufferw)) < 0 ) printMsg("ERROR writing to socket");
#define SOCK_READ(a) bzero(a, 256) ; \
    if (read(sockfd,a,255) < 0)	printMsg("ERROR reading from socket");
*/    
