...
int init_socket() {
  MSGQUEUEOPTIONS opt;
  char * s1 = "phalanx2scid";
  char * s2 = "scid2phalanx";
  LPWSTR sunicode;
...
// ======================================================================
// Messages are written only when \n is present (\n is not sent)
void send_msg(const char format[], ...) {
  ...
  if ( string[strlen(string)-1] != '\n' ) {
    strcat(send_msg_buffer, string);
    return;
  } 

  string[strlen(string)-1] = '\0';

  if (send_msg_buffer[0] != '\0') {
    strcat( send_msg_buffer, string );
    if (!WriteMsgQueue( hw, (LPVOID)send_msg_buffer, strlen(send_msg_buffer), 0, 0 ) );
  } else {
...
//   string[strlen(string)-1] = '\0';
  ...
  if (send_msg_buffer[0] != '\0') {
    strcat( send_msg_buffer, string );
   if (write(sockfd, send_msg_buffer,strlen(send_msg_buffer)) < 0 ) printf("ERROR writing to socket\n");
  } else {
